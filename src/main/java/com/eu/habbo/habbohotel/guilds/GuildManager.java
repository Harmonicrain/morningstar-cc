...
public class GuildManager {
    ...
    public ArrayList<GuildMember> getGuildMembers(Guild guild, int page, int levelId, String query) {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("SELECT users.username, users.look, guilds_members.* FROM guilds_members INNER JOIN users ON guilds_members.user_id = users.id WHERE guilds_members.guild_id = ?  " + (rankQuery(levelId)) + " AND users.username LIKE ? ORDER BY level_id ASC, member_since DESC LIMIT ?, ?")) {
            statement.setInt(1, guild.getId());
    ...
    public int getGuildMembersCount(Guild guild, int page, int levelId, String query) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection(); PreparedStatement statement = connection.prepareStatement("SELECT COUNT(*) FROM guilds_members INNER JOIN users ON guilds_members.user_id = users.id WHERE guilds_members.guild_id = ?  " + (rankQuery(levelId)) + " AND users.username LIKE ?")) {
            statement.setInt(1, guild.getId());
            statement.setString(2, "%" + query + "%");
