...
/** July Invisible Furni Controller (`conf_invis_control`). */
public class InteractionInvisControl extends HabboItem {
    ...
    public InteractionInvisControl(int id, int userId, Item item, String extradata,
    ...
    public static boolean isInvisibleFurniHidden(Room room) {
        if (room == null) {
            return false;
        }
        for (HabboItem item : room.getFloorItems()) {
            if (item instanceof InteractionInvisControl && ((InteractionInvisControl) item).isEnabled()) {
                return true;
            }
        }
        return false;
